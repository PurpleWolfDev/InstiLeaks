    ...
    return(
        <>
            <div className="__pagi_container">
                ...
                <div className={"__pagi_eachOpt "+(currentPage==1?"__pagi_selected":"")} onClick={() => {dispatch(filterTabs({query:'confession'}));currentPage!=1&&navigate("/confession");}}><p>{currentPage==1?<img className='__pagi_img' src={filledUser} />:<img className='__pagi_img' src={outlineUser} />}</p></div>
                <div className={"__pagi_eachOpt "+(currentPage==2?"__pagi_selected":"")} onClick={() => {currentPage!=2&&navigate("/createPost")}}><p>{currentPage==2?<IoAddCircle size={24} />:<IoAddCircleOutline size={24}/>}</p></div>
                <div className={"__pagi_eachOpt "+(currentPage==3?"__pagi_selected":"")} onClick={() => {dispatch(filterTabs({query:'general'}));currentPage!=3&&navigate("/foodRadar")}}><p>{currentPage==4?<IoFastFoodSharp />:<IoFastFoodOutline />}</p></div>
